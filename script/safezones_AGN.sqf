...
Private ["_EH_Fired", "_ehID", "_fix","_inVehicle","_inVehicleLast","_EH_Fired_Vehicle",
		"_inVehicleDamage","_antiBackpackThread","_antiBackpackThread2","_anti_zombie",
		"_vehicle","_veh_array","_y","_veh_total"];
...
if ( AGN_safeZoneMessages ) then { systemChat ( "Script Trader Safe Zone chargé!" ); };
...
while {true} do {
	...
	if ( AGN_safeZoneMessages ) then { systemChat ("Vous entrez en zone safe - Protection activée"); };
	_thePlayer = player;
	...
	if ( AGN_safeZone_Players_DisableWeaponFiring ) then
	{
		_EH_Fired = _thePlayer addEventHandler ["Fired", {
			systemChat ("Vous ne pouvez pas tirer en zone safe");
			...
			while {!canBuild} do
			{
				...
				if( !isNull (FindDisplay 106) && !_skip ) then
				{
					if ( isNull AGN_LastPlayerLookedAt ) then
					{
						...
						if ( AGN_safeZoneMessages ) then { systemChat ("Protection contre le vol - Redirection vers votre inventaire!"); };
						waitUntil { isNull (FindDisplay 106) };
					} else {
						if ( AGN_safeZoneMessages ) then { systemChat (format["Vous ne pouvez pas encore ouvrir votre inventaire. Patientez encore 5 secondes."]); };
						(findDisplay 106) closeDisplay 1;
						waitUntil { isNull (FindDisplay 106) };
					};
				};
				if ( _skip && _if ) then {
					if ( AGN_safeZoneMessages ) then { systemChat ("Ce joueur est taggué friendly, vous pouvez donc accéder à son inventaire.") };
				};
			};
		};
	};
	...
	if ( AGN_safeZone_Vehicles_DisableMountedGuns ) then
	{
		while { !canBuild } do
		{
			...
			{
				if (AGN_safeZoneMessages) then { systemChat ( "Désactivation des véhicules armés" ); };
				_inVehicle = vehicle player;
				_inVehicleDamage = getDammage _inVehicle;
				_EH_Fired_Vehicle = _inVehicle addEventHandler ["Fired", {
					systemChat ("Vous ne pouvez pas utiliser l'arme du véhicule en zone safe!");
					NearestObject [_this select 0,_this select 4] setPos[0,0,0];
				}];
			};
		};
	...
	if ( AGN_safeZoneMessages ) then { systemChat ("Vous quittez la zone safe - Protection désactivée"); };
	
	if ( AGN_safeZone_Vehicles_DisableMountedGuns ) then
	{
		if ( !(isNull _inVehicle) ) then
		{
			if ( AGN_safeZoneMessages ) then { systemChat ( "Réactivation des véhicules armés" ); };
			_inVehicle removeEventHandler ["Fired", _EH_Fired_Vehicle];
		};
