...
call compile preprocessFileLineNumbers "script\EVD\EVD_common.sqf";

if (dayz_combat == 1) exitwith { cutText ["\n\nVeuillez attendre la fin du mode combat !", "PLAIN DOWN"] };
...
if (_canDo) then {
	...
	if (_proceed) then {
		...
		cutText ["\n\nDébut de construction du véhicule", "PLAIN DOWN"];
		...
		if (_finished) then {
			...
			if(_removed_total == _tobe_removed_total) then {
				...
				_object addEventHandler ["GetIn",{
					_nil = [nil,(_this select 2),"loc",rTITLETEXT,"Le véhicule disparaitra au restart!","PLAIN DOWN",5] call RE;
				}];
				...
				cutText ["\n\nConstruction du vehicule OK !", "PLAIN DOWN"];
			} else {
			...
			cutText ["\n\nDeployement du véhicule annulé ", "PLAIN DOWN"];
		};
	} else {
