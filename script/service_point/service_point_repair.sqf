...
_allRepaired = true;
{
	...
	if ((vehicle player != _vehicle) || (!local _vehicle) || ([0,0,0] distance (velocity _vehicle) > 1)) exitWith {
		_allRepaired = false;
		titleText [format["Réparation %1 arrêtée", _name], "PLAIN DOWN"];
	};
	...
	if (_damage > 0) then {
		if (_repairTime > 0) then {
			...
			titleText [format["Réparation %1 ...", _partName], "PLAIN DOWN", _repairTime];
			sleep _repairTime;
		};
...
if (_allRepaired) then {
	...
	titleText [format["%1 Réparé", _name], "PLAIN DOWN"];
};
