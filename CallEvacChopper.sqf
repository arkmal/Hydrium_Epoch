...
{
	systemChat(format ["Evac-Chopper sera appelé dans %1s - Move to cancel!",_cnt]);
...
systemChat ("Recherche du Evac-Chopper - Please wait...");
sleep 5;
...
if ((count _checkForChopper) > 0) then {
	...
	systemChat ("No Chopper on your Evac-Field");
	s_player_evacCall = -1;
	breakOut "exit";
};
...
{			
	...
	if (_damage >= 1 && (_part == "PartEngine" || _part == "PartVRotor")) then {
		if(_part == "PartEngine") then {
			systemChat("Sorry but the Engine is to damaged to fly");
			systemChat("Keep your Evac-Chopper repaired next time!");
		};
		if (_part == "PartVRotor") then {
			systemChat("Sorry but the Main-Rotor is to damaged to fly");
			systemChat("Keep your Evac-Chopper repaired next time!");
		};
		sleep 30;
		s_player_evacCall = -1;
